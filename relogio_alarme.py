...
def alarme(alarme_programado):
    while True:
        ...
        if hora == alarme_programado:
            Alarme = Label(relogio, text="ALARME !!!", fg="red", bg="black", relief="solid",
                           font=("Helevetica", 7, "bold")).place(x=300, y=29)
            print("ALARME !!!")
...
formato_tempo = Label(relogio, text="Inserir horário em formato 24h", fg="blue", relief="solid", font=("Arial")).place(
    x=60, y=120)
adicaoTempo = Label(relogio, text="Hora / Min / Seg", font=60).place(x=110)
configAlarme = Label(relogio, text="Horário para tocar:", fg="blue", relief="solid",
...
# Tempo requerido para modificar horário de alarme - Entrada dados
...
seg_tempo = Entry(relogio, textvariable=seg, bg='pink', width=15).place(x=190, y=30)

# Ler o tempo inserido pelo usuário:
submit = Button(relogio, text="Set Alarm", fg="green", width=10, command=hora_atual).place(x=110, y=70)

# Botão resetar
finish = Button(relogio, text="Finish", fg="red", width=10, command=resetar).place(x=200, y=70)
